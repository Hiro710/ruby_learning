...
# 配列の演算
# 「|」は和集合を求めるメソッドで、「&」は積集合を求めるメソッド
p [1, 2, 3] | [1, 3, 5]   #=> [1, 2, 3, 5]
p [1, 2, 3] & [1, 3, 5]   #=> [1, 3]

# 配列の比較
# 配列同士の比較には==を使う
# <=>は先頭から比較し、同じであれば0を、左辺が大きければ1を、右辺が大きければ-1を返す
p [1, 2, 3] == [1, 3, 5]  #=> false
p [1, 2, 3] <=> [1, 3, 5] #=> -1
...
=begin
  ...
  eachはブロックに各要素が、each_indexは配列のインデックスが渡る。
  reverse_eachは要素の逆順に繰り返す
=end

p [1, 3, 5, 7, 9].each { |n| puts n * 2 }
=begin
  2
  6
  10
  ...
  [1, 3, 5, 7, 9]
=end
p [1, 3, 5, 7, 9].each_index { |n| puts n * 2 } # 配列のインデックス番号がnに代入される
=begin
  0
  2
  4
...
# cycleは配列の要素を順に繰り返し、末尾まできたらまた先頭に戻って繰り返し続ける
# p [1, 2, 3].cycle { |n| puts n }
=begin
  1
  2
  3
...
# 配列の要素を連結する
# joinと「*」は配列の要素を指定した区切り文字で連結した文字列を返す。
# to_sは区切り文字として「$,」を指定したjoinと同じ動作をする
p [1, 2, 3].join(", ")    #=> "1, 2, 3"

# 配列の長さを求める(length, size)
# 配列が空の場合は0を返す
# empty?は配列が空の場合に真を返す
p [1, 2, 3].length    #=> 3
p [].length           #=> 0
...
# 配列をソートする
# sortを使う、要素同士の比較には<=>を使い、
# メソッドの末尾に!が付いている場合は自身をソート結果で書き換える(破壊的メソッド)
a = [1, 3, 5, 2, 4, 6]
p a.sort        #=> [1, 2, 3, 4, 5, 6]
p a             #=> [1, 3, 5, 2, 4, 6]
p a.sort!       #=> [1, 2, 3, 4, 5, 6]
p a             #=> [1, 2, 3, 4, 5, 6]

=begin
  ブロックが与えられた場合には2つの要素が渡され、その評価結果に応じてソートされる
  評価結果が正の整数の場合には1つ目の要素が大きく、0であれば同じ、
  負の整数であれば2つ目の要素が大きいとしてソートされる
=end

p [1, 3, 5, 2, 4, 6].sort { |a, b| a <=> b }    #=> [1, 2, 3, 4, 5, 6]
p [1, 3, 5, 2, 4, 6].sort { |a, b| b <=> a }    #=> [6, 5, 4, 3, 2, 1]
...
=begin
  ...
  それぞれメソッドの末尾に「!」が付くと破壊的メソッドになり、自身の値を書き換える
=end

# uniqは配列から重複した要素を取り除いた配列を返す
# uniq!の場合は取り除かれなかった場合にnilを返す
p [1, 1, 2, 3, 3].uniq                       #=> [1, 2, 3]
p [1, 2, 3].uniq!                            #=> nil

# compactは要素のnilを取り除いた配列を返す
# compact!の場合は取り除かれなかった場合にnilを返す
p [1, nil, 2, nil, 3].compact                #=> [1, 2, 3]
p [1, 2, 3].compact!                         #=> nil

# revereseは配列の要素を逆順に並べ替えた配列を返す
p [1, 2, 3, 4, 5].reverse                    #=> [5, 4, 3, 2, 1]

# flattenは配列を再帰的に平滑化した配列を返す
# 引数が指定された場合は、その深さまで再帰的に平滑化する
# flatten!の場合には平滑化されなかった場合にnilを返す
p [[[1, 2,], 3], [[4, 5], 6]].flatten        #=> [1, 2, 3, 4, 5, 6]
p [[[1, 2,], 3], [[4, 5], 6]].flatten(1)     #=> [[1, 2], 3, [4, 5], 6]
p [1, 2, 3, 4, 5, 6].flatten!                #=> nil

# mapとcollectは、要素ごとにブロックを評価し、その結果で要素を置き換えた配列を返す
 p [1, 2, 3, 4, 5].map { |n| n * 2 }         #=> [2, 4, 6, 8, 10]
 p [1, 2, 3, 4, 5].collect { |n| n * 2 }     #=> [2, 4, 6, 8, 10]

# shuffleは配列の要素をシャッフルして返す
p [1, 2, 3, 4, 5].shuffle                    #=> [5, 3, 4, 2, 1](一例)

# 配列を組み合わせて生成する(product, zip, *)
# productは自身と与えられた配列から1つずつ要素を取って組み合わせた配列を作り、
# その全ての組み合わせを要素とする配列を返す
p [1, 2].product(["a", "b", "c"])            #=> [[1, "a"], [1, "b"], [1, "c"], [2, "a"], [2, "b"], [2, "c"]]

# zipは自身と与えられた配列から1つずつ要素を取って配列を作り、それを要素とする配列を返す
# productとは異なり、組み合わせは前から順に同じインデックスのもののみとなる
# ブロックが与えられた場合には、自身と引数に指定された配列を順にブロックに渡す
p [1, 2].zip(["a", "b"])                     #=> [[1, "a"], [2, "b"]]
p [1, 2].zip(["a", "b"], ["x", "y"])         #=> [[1, "a", "x"], [2, "b", "y"]]

# 「*」は与えられた数値の分だけ繰り返した配列を返す
p [1, 2] * 3                                 #=> [1, 2, 1, 2, 1, 2]

# 配列をパックする
# packは自身を指定されたテンプレートに従ってパックする
# packはMIMEエンコードする場合に利用できる
p ["ルビー"].pack('m')                        #=> "44Or44OT44O8\n"