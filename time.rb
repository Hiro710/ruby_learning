...
# to_iとtv_secは起算時からの秒数を整数で、to_fは浮動小数点数で返す
t = Time.mktime(2017, 1, 2, 3, 4, 5, 6)
p t.to_i            #=> 1483293845
p t.tv_sec          #=> 1483293845
p t.to_f            #=> 1483293845.000006

# to_aは先頭から、秒、分、時、日、月、年、曜日に対応する数値、年日、夏時間かどうかの真理値、
# タイムゾーンの順の配列を返す
t = Time.mktime(2017, 1, 2, 3, 4, 5, 6)
p t.to_a            #=> [5, 4, 3, 2, 1, 2017, 1, 2, false, "JST"]

# to_sは、UNIXコマンドのdateコマンドのような形式の文字列を返す
t = Time.mktime(2017, 1, 2, 3, 4, 5, 6)
p t.to_s            #=> "2017-01-02 03:04:05 +0900"

# strftimeは、指定したフォーマットに従って文字列に変換した結果を返す
t = Time.mktime(2017, 1, 2, 3, 4, 5, 6)
puts t.strftime("%Y年%m月%d日 %H時%M分%S秒")      #=> 2017年01月02日 03時04分05秒